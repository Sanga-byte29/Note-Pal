...
export async function generateImagePrompt(name: string) {
  try {
    ...
    if (choices && choices.length > 0) {
      ...
      console.log("Invalid data structure or empty choices array.");
      // Handle the situation where the data structure is unexpected or choices array is empty.
      return ""; // or throw an error, or handle it based on your application logic
    }
  } catch (error) {
    console.log(error);
    throw error;
